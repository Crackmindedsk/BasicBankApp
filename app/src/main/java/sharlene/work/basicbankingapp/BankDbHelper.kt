...
class BankDbHelper(context: Context) : SQLiteOpenHelper(context, databaseName,null, version) {
    ...
    override fun onCreate(db: SQLiteDatabase?){
        val CREATE="CREATE TABLE user (_id INTEGER, name TEXT, email VARCHAR PRIMARY KEY, balance INTEGER, phone TEXT, account VARCHAR)"
        db?.execSQL(CREATE)
        val createTransact="CREATE TABLE transact (transact_id INTEGER, fromName TEXT, toName TEXT, amount INTEGER, status TEXT)"
